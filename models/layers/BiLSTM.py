...
class BiLSTM(nn.Module):
    def __init__(self, D: int, hidden_size: int,
                 input_dropconnect: float = 0.0,
                 hidden_dropconnect: float = 0.0,
                 zoneout: float = 0.0,
        ...
        # input dropconnect: enables dropconnect on input weights (should have a similar effect to variational input dropout but more efficient)
        # hidden dropconnect: enables dropconnect on hidden weights (should have a similar effect to variational hidden dropout but more efficient)
        # zoneout = enables "zoneout" (google it) regularization on hidden states
        # paraneterized_states = decides whether to treat initial hidden and cell state as fixed untrainable zero initialized vectors (if False), or treat them as trainable parameters (if True)
